...
export class PortfolioService extends TypeOrmCrudService<Portfolio> {
  ...
  constructor(
    ...
    private readonly portfolioAssessRepo: Repository<PortfolioAssessment>,
    @InjectRepository(InvestorAssessment)
    private readonly investorAssessRepo: Repository<InvestorAssessment>,
    @InjectRepository(SdgAssessment)
    private readonly sdgAssessRepo: Repository<SdgAssessment>,
    @InjectRepository(Assessment)
    private readonly assessmentRepo: Repository<Assessment>,
    @InjectRepository(SdgAssessment)
    private readonly sdgAssessmentRepo: Repository<SdgAssessment>,
    @InjectRepository(Results)
    private readonly resultsRepo: Repository<Results>,
  ...
  async getAll(): Promise<Portfolio[]> {
    ...
    for (const x of await res) {
      ...
      const isMatchingCountry =
        x.user?.country?.id === currentUser?.country?.id;
  ...
  async assessmentsDataByAssessmentId(portfolioId: number): Promise<any[]> {
    ...
    for (let data of await response) {
      ...
      categoriesMap.forEach((category) => {
        category.likelihoodAverage = (
          category.totalLikelihood / category.totalCharacteristics
        )?.toFixed(0);
        category.relevanceAverage = (
          category.totalRelevance / category.totalCharacteristics
        )?.toFixed(0);
        category.scoreAverage = (
          category.totalScore / category.totalCharacteristics
        )?.toFixed(0);
      });
  ...
  async getPortfolioComparisonData(portfolioId: number) {
    ...
    for (let pAssessment of pAssessments) {
      let score = await this.cMAssessmentQuestionService.calculateResult(
        pAssessment.assessment.id,
      );
      let piscore =
        await this.investorToolService.calculateNewAssessmentResults(
          pAssessment.assessment.id,
        );
    ...
    response.process_data = await this.getProcessData(
      pAssessments,
      cmScores,
      piScores,
    );
    let outcomeResponse = await this.getOutcomeData(
      pAssessments,
      cmScores,
      piScores,
    );
    ...
    response.alignment_data = await this.getAlignmentData(
      pAssessments,
      cmScores,
      outcomeResponse.sdg_scores,
    );
  ...
  async getProcessData(
    pAssessments: PortfolioAssessment[],
    cmScores: any,
    piScores: any,
  ) {
    ...
    for await (let pAssessment of pAssessments) {
      ...
      switch (pAssessment.assessment.tool) {
        case 'PORTFOLIO':
        case 'INVESTOR':
          res = await this.getProcessDataPortfolioInvestor(
            pAssessment.assessment,
            piScores,
          );
          break;
        case 'CARBON_MARKET':
          res = await this.getProcessDataCarbonMarket(
            pAssessment.assessment,
            cmScores,
          );
          break;
    ...
    for (let [idx, cat] of int_categories.entries()) {
      likelihood_comparison.col_set_1.push({
        label: 'Category - ' + cat,
        colspan: 1,
      });
      likelihood_comparison.col_set_2.push({
      ...
      for (let [index, int_data] of intervention_data.entries()) {
        ...
        if (data) {
          data.col_set_1 = {
            ...data.col_set_1,
            label: 'Category - ' + data.col_set_1.label,
          };
          if (comparisonData.col_set_1.length === 1)
            comparisonData.col_set_1.push(data.col_set_1);
          if (index === 0) {
            comparisonData.col_set_2.push(...data.characteristics);
            comparisonData.characteristic_count = data.characteristic_count;
          }
          comparisonData.interventions.push({
    ...
    likelihood_comparison.col_set_2.push({
      label: 'Processes score',
      code: 'category_score',
    });
    for (let int of intervention_data) {
      likelihood_comparison.interventions.push({
        ...int.intervention,
        ...int.category_scores,
      });
    }
  ...
  async getOutcomeData(
    ...
    for (let pAssessment of pAssessments) {
      ...
      switch (pAssessment.assessment.tool) {
        case 'PORTFOLIO':
        case 'INVESTOR':
          data = await this.getOutcomeDataPortfolioInvestor(
            pAssessment.assessment,
            piScores,
          );
          break;
        case 'CARBON_MARKET':
          data = await this.getOutcomeDataCarbonMarket(
            pAssessment.assessment,
            cmScores,
          );
          break;
    ...
    for (let [index, int_data] of intervention_data.entries()) {
      if (index === 0) {
        scaleGhgData.col_set_1 = [
          ...col_set_1,
          int_data.data.scale_comparisons.ghg.col_set_1,
        ];
        scaleGhgData.col_set_2 = [...this.col_set_2, ...col_set_2_scale];
        scalAdaptationData.col_set_1 = [
          ...col_set_1,
          int_data.data.scale_comparisons.adaptation.col_set_1,
        ];
        scalAdaptationData.col_set_2 = [...this.col_set_2, ...col_set_2_scale];
        sustainedGhgData.col_set_1 = [
          ...col_set_1,
          int_data.data.sustained_comparisons.ghg.col_set_1,
        ];
        sustainedGhgData.col_set_2 = [
          ...this.col_set_2,
          ...col_set_2_sustained,
        ];
        sustainedAdaptationData.col_set_1 = [
          ...col_set_1,
          int_data.data.sustained_comparisons.adaptation.col_set_1,
        ];
        sustainedAdaptationData.col_set_2 = [
          ...this.col_set_2,
          ...col_set_2_sustained,
        ];
      }
      for (let sd of sdgs) {
        if (int_data.data.sdg.includes(sd)) {
          if (scaleSdgData[sd].col_set_1.length === 0)
            scaleSdgData[sd].col_set_1 = [
              ...col_set_1,
              int_data.data.scale_comparisons.sdg[sd].col_set_1,
            ];
          if (sustainedSdgData[sd].col_set_1.length === 0)
            sustainedSdgData[sd].col_set_1 = [
              ...col_set_1,
              int_data.data.sustained_comparisons.sdg[sd].col_set_1,
            ];
        }
      }

      outcome_score[int_data.intervention.assessment_id] =
        int_data.data.outcome_score;
      scaleGhgData.interventions.push({
        ...int_data.intervention,
        ...int_data.data.scale_comparisons.ghg.data,
      });
      order = 1;
      sus_order = 2 + sdgs.length;
      for (let sd of sdgs) {
        if (
          int_data.data.sdg.includes(sd) &&
          int_data.data.scale_comparisons.sdg[sd]
        ) {
          order += 1;
          sus_order += 1;
          scaleSdgData[sd].interventions.push({
            ...int_data.intervention,
            ...int_data.data.scale_comparisons.sdg[sd].data,
          });
          sustainedSdgData[sd].interventions.push({
            ...int_data.intervention,
            ...int_data.data.sustained_comparisons.sdg[sd].data,
          });
        }
      }

      scalAdaptationData.interventions.push({
        ...int_data.intervention,
        ...int_data.data.scale_comparisons.adaptation.data,
      });
      order += 1;

      sustainedGhgData.interventions.push({
        ...int_data.intervention,
        ...int_data.data.sustained_comparisons.ghg.data,
      });
      sus_order += 1;
      sustainedAdaptationData.interventions.push({
        ...int_data.intervention,
        ...int_data.data.sustained_comparisons.adaptation.data,
      });
    }

    scale_comparison.comparison_type = 'Scale comparison';
    scale_comparison.comparison_type_2 = 'Outcomes';
    scale_comparison.col_set_1 = [...col_set_1, { label: 'GHG', colspan: 1 }];
    ...
    sc_sus_ghg_comparison.comparison_type =
      'Scale & Sustained in time comparison';
    sc_sus_ghg_comparison.comparison_type_2 = 'GHG Outcomes';
    sc_sus_ghg_comparison.col_set_1 = [...col_set_1, { label: '', colspan: 3 }];
    ...
    scaleGhgData.interventions.map((int) => {
      ...
      sc_sus_ghg_comparison.interventions.push({
        ...
        status: int.status,
        assessment_id: int.assessment_id,
        scale_score: int.category_score,
      });

      scale_cat_total[int.assessment_id] =
        int.category_score.value === null ? null : int.category_score.value;
      scale_cat_count[int.assessment_id] = 1;
      sc_cat_total[int.assessment_id] =
        int.category_score.value === null ? null : int.category_score.value;
      sc_sus_total[int.assessment_id] =
        int.category_score.value === null ? null : int.category_score.value;
    });
    ...
    sustainedGhgData.interventions.map((int) => {
      ...
      let res = sc_sus_ghg_comparison.interventions.find(
        (o) => o.assessment_id === int.assessment_id,
      );
    ...
    for (let [index, sd] of sdgs.entries()) {
      scale_comparison.col_set_1.push({
        label: scaleSdgData[sd].col_set_1[1].label,
        colspan: 1,
      });
      scale_comparison.col_set_2.push({
        label: 'Catogory score',
        code: sd + '_score',
      });

      sustained_comparison.col_set_1.push({
        label: sustainedSdgData[sd].col_set_1[1].label,
        colspan: 1,
      });
      ...
      sc_sus_sdgs[sd].comparison_type_2 =
        'SDG Outcomes - ' + label[1].trim() + ' - ' + label[2].trim();
      sc_sus_sdgs[sd].col_set_1 = [...col_set_1, { label: '', colspan: 3 }];
      ...
      scaleSdgData[sd].interventions.map((int) => {
        let res = scale_comparison.interventions.find(
          (o) => o.assessment_id === int.assessment_id,
        );
        if (res) {
      ...
      sustainedSdgData[sd].interventions.map((int) => {
        let res = sustained_comparison.interventions.find(
          (o) => o.assessment_id === int.assessment_id,
        );
        if (res) {
          res[sd + '_score'] = int.category_score;
        }
        let res2 = sc_sus_sdgs[sd].interventions.find(
          (o) => o.assessment_id === int.assessment_id,
        );
        if (res2) {
      ...
      sc_sus_sdgs[sd].interventions = sc_sus_sdgs[sd].interventions.map(
        (int) => {
          let score = null;
    ...
    scale_comparison.col_set_1.push(
      ...
      { label: 'Scale catogory score', colspan: 1 },
    );
    ...
    sustained_comparison.col_set_1.push(
      { label: 'Adaptation', colspan: 1 },
      { label: 'Sustained catogory score', colspan: 1 },
    );
    sustained_comparison.col_set_2.push(
      ...[
        { label: 'Catogory score', code: 'adaptation_score' },
    ...
    sc_sus_ad_comparison.comparison_type =
      'Scale & Sustained in time comparison';
    sc_sus_ad_comparison.comparison_type_2 = 'Adaptation Outcomes';
    sc_sus_ad_comparison.col_set_1 = [...col_set_1, { label: '', colspan: 3 }];
    ...
    scalAdaptationData.interventions.map((int) => {
      ...
      sc_cat_total[int.assessment_id] === null
        ? int.category_score.value === null
          ...
          : (sc_cat_total[int.assessment_id] = int.category_score.value)
        : (sc_cat_total[int.assessment_id] += int.category_score.value);
      sc_sus_ad_total[int.assessment_id] =
        int.category_score.value === null ? null : int.category_score.value;
    });
    ...
    sustainedAdaptationData.interventions.map((int) => {
      ...
      let res2 = sc_sus_ad_comparison.interventions.find(
        (o) => o.assessment_id === int.assessment_id,
      );
      if (res2) {
        res2['sustained_score'] = int.category_score;
    ...
    scale_comparison.interventions = scale_comparison.interventions.map(
      (int) => {
        let score =
          sc_cat_total[int.assessment_id] !== null
            ? floorToHalf(
    ...
    sustained_comparison.interventions = sustained_comparison.interventions.map(
      (int) => {
        let score =
          ss_cat_total[int.assessment_id] !== null
            ? floorToHalf(
            ...
            : null;
        int['category_score'] = this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(score),
          score,
        );
        return int;
      },
    ...
    sc_sus_ghg_comparison.interventions =
      sc_sus_ghg_comparison.interventions.map((int) => {
        let score = null;
        if (int.scale_score.value !== null) {
          score = int.scale_score.value;
        }
    ...
    sc_sus_ad_comparison.interventions = sc_sus_ad_comparison.interventions.map(
      (int) => {
        let score = null;
        if (int.scale_score.value !== null) {
          score = int.scale_score.value;
    ...
    scaleGhgData.interventions.map((int) => {
      ...
      outcome_level_comparison.interventions.push({
        ...
        tool: int.tool,
        status: int.status,
        scale_cat_score: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scale_score),
          scale_score,
        ),
        sustained_cat_score: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(sustaine_score),
          sustaine_score,
        ),
        category_score:
          outcome_score[int.assessment_id] === null
            ? 'N/A'
            : this.mapNameAndValue(
                this.investorToolService.mapScaleScores(
                  outcome_score[int.assessment_id],
                ),
                outcome_score[int.assessment_id],
              ),
      });
    ...
    response.push(
      ...
      scalAdaptationData,
      sustainedGhgData,
      sustainedAdaptationData,
      scale_comparison,
      sustained_comparison,
      sc_sus_ghg_comparison,
      sc_sus_ad_comparison,
      outcome_level_comparison,
    );
  ...
  async getAggragationData(pAssessments: PortfolioAssessment[]) {
    ...
    for (let pAssessment of pAssessments) {
      ...
      switch (pAssessment.assessment.tool) {
        case 'PORTFOLIO':
          _intervention.mitigation = await this.getAggregationDataPortfolioTool(
            pAssessment.assessment,
          );
        ...
        case 'INVESTOR':
          ...
          break;
        case 'CARBON_MARKET':
          _intervention.mitigation = await this.getAggregationDataCarbonMarket(
            pAssessment.assessment,
          );
          total += _intervention.mitigation;
          response.interventions.push(_intervention);
      ...
      _intervention.mitigation = this.thousandSeperate(
        _intervention.mitigation,
        4,
      );
    }
  ...
  async getAlignmentData(
    ...
    let userCountryId = currentUser.country?.id;
    let sdgPriorities =
      await this.investorToolService.getSdgPrioritiesByCountryId(userCountryId);
    for (let pAssessment of pAssessments) {
      let _intervention = {
        id: pAssessment.assessment.climateAction.intervention_id,
        name: pAssessment.assessment.climateAction.policyName,
      ...
      switch (pAssessment.assessment.tool) {
        case 'PORTFOLIO':
        case 'INVESTOR':
          data = await this.getAlignmentDataPortfolioInvestor(
            pAssessment.assessment,
            sdgPriorities,
            sdgs_score[pAssessment.assessment.id.toString()],
          );
          break;
        case 'CARBON_MARKET':
          data = await this.getAlignmentDataCarbonMarket(
            pAssessment.assessment,
            cmScores,
            sdgPriorities,
          );
    ...
    response.col_set_1 = [{ label: 'Intervention information', colspan: 4 }];
    response.col_set_2 = [...this.col_set_2];

    for (let int_data of intervention_data) {
      response.interventions.push({
        ...int_data.intervention,
        ...int_data.data.response,
      });
    ...
    response.col_set_2 = response.col_set_2.reduce(
    ...
    response.col_set_1 = response.col_set_1.reduce(
      (uniqueArray, currentItem) => {
  ...
  async getProcessDataPortfolioInvestor(assessment: Assessment, piScores: any) {
    ...
    cat_obj['category_score'] =
      res.processScore === null ? 'N/A' : res.processScore;

    return {
      categories: categories,
      cat_names: cat_names,
      category_scores: cat_obj,
    };
  }
  ...
  async getOutcomeDataPortfolioInvestor(assessment: Assessment, piScores: any) {
    ...
    let int = scGHG.characteristicData.find((o) => o.ch_code === 'MACRO_LEVEL')
      ?.score;
    let nat = scGHG.characteristicData.find((o) => o.ch_code === 'MEDIUM_LEVEL')
      ?.score;
    let sub = scGHG.characteristicData.find((o) => o.ch_code === 'MICRO_LEVEL')
      ?.score;
    ...
    let scale_GHGs = {
      ...
      data: {
        ...
        category_score: this.mapNameAndValue(
          scGHG.category_score?.name,
          scGHG.category_score?.value,
        ),
      },
    };
    ...
    for (let sd of scSD.characteristicData) {
      ...
      sdg.push(sd_code);
      let international = sd.data.find(
        (o) => o.ch_code === 'MACRO_LEVEL',
      ).score;
      ...
      scale_SDs[sd_code] = {
        ...
        data: {
          international: this.mapNameAndValue(
            international?.name,
            international?.value,
          ),
          national: this.mapNameAndValue(national?.name, national?.value),
          subnational: this.mapNameAndValue(
            subnational?.name,
            subnational?.value,
          ),
          category_score: this.mapNameAndValue(
            this.investorToolService.mapScaleScores(sd.sdg_score),
            sd.sdg_score,
          ),
        },
      };
    ...
    int = scAD.characteristicData.find((o) => o.ch_code === 'INTERNATIONAL')
      ?.score;
    nat = scAD.characteristicData.find((o) => o.ch_code === 'NATIONAL')?.score;
    sub = scAD.characteristicData.find((o) => o.ch_code === 'SUBNATIONAL')
      ?.score;
    ...
    let scale_adaptation = {
      ...
      data: {
        ...
        subnational: this.mapNameAndValue(sub?.name, sub?.value),
        category_score: this.mapNameAndValue(
          scAD.category_score?.name,
          scAD.category_score?.value,
        ),
      },
    };
    ...
    let _long = susGHG.characteristicData.find((o) => o.ch_code === 'LONG_TERM')
      ?.score;
    let medium = susGHG.characteristicData.find(
      (o) => o.ch_code === 'MEDIUM_TERM',
    )?.score;
    let short = susGHG.characteristicData.find(
      (o) => o.ch_code === 'SHORT_TERM',
    )?.score;
    ...
    let sustained_GHGs = {
      ...
      data: {
        ...
        category_score: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(
            susGHG.category_score?.value,
          ),
          susGHG.category_score?.value,
        ),
      },
    ...
    for (let sd of susSD.characteristicData) {
      ...
      sustained_SDs[sd_code] = {
        ...
        data: {
          ...
          medium_term: this.mapNameAndValue(
            medium_term.name,
            medium_term.value,
          ),
          short_term: this.mapNameAndValue(short_term.name, short_term.value),
          category_score: this.mapNameAndValue(
            this.investorToolService.mapSustainedScores(sd.sdg_score),
            sd.sdg_score,
          ),
        },
      };
    ...
    _long = susAD.characteristicData.find((o) => o.ch_code === 'INTERNATIONAL')
      ?.score;
    medium = susAD.characteristicData.find((o) => o.ch_code === 'NATIONAL')
      ?.score;
    short = susAD.characteristicData.find((o) => o.ch_code === 'SUBNATIONAL')
      ?.score;
    ...
    return {
      scale_comparisons: {
        ghg: scale_GHGs,
        sdg: scale_SDs,
        adaptation: scale_adaptation,
      },
      sustained_comparisons: {
        ghg: sustained_GHGs,
        sdg: sustained_SDs,
        adaptation: sustained_adaptation,
      },
      sdg: sdg,
  ...
  async getAlignmentDataPortfolioInvestor(
    ...
    let col2 = [];
    let sdgsArr = [];
    let data = this.sdgAssessRepo
      .createQueryBuilder('sdgAssessment')
      .innerJoin(
    ...
    sdgs.map((sd) => {
      ...
      if (sdgPriorities.length !== 0) {
        ...
        let priority_name = this.masterDataService.sdg_priorities.find(
          (o) => o.code === priority.priority,
        )?.name;
        priority_value =
          ans.value === null ? null : 4 - Math.abs(ans.value - priority.value);
        col2.push({ label: priority_name, code: code });
      }
      response[code] = { name: ans?.name, value: priority_value };
      col1.push({
        label: 'SDG ' + sd.sdg.number + ' - ' + sd.sdg.name,
        colspan: 1,
      });
  ...
  async getAggregationDataPortfolioTool(assessment: Assessment) {
    let result = await this.assessmentRepo
      .createQueryBuilder('asses')
      .leftJoinAndMapMany(
      ...
      .where(
        'asses.id=:assessmentid  and investor_assessment.characteristic_id in (16,28)',
    ...
    if (result.investor_assessment) {
      for (let investor of result.investor_assessment) {
        if (assessment.tool === 'INVESTOR') {
          investor.expected_ghg_mitigation_year
            ? (total += Number(investor.expected_ghg_mitigation_year))
            : 0;
        } else {
          investor.expected_ghg_mitigation
            ? (total += Number(investor.expected_ghg_mitigation))
            : 0;
        }
      }
    }
  ...
  async getProcessDataCarbonMarket(assessment: Assessment, cmScores: any) {
    let data = (
      await this.cMAssessmentQuestionService.getProcessData(assessment.id)
    ).data;
    ...
    cat_obj['category_score'] =
      result.process_score === null ? 'N/A' : result.process_score;
  ...
  async getOutcomeDataCarbonMarket(assessment: Assessment, cmScores: any) {
    let data = (
      await this.cMAssessmentQuestionService.getResults(assessment.id)
    )?.outComeData;
    let result = cmScores[assessment.id];

    let scGHG_int = data.scale_GHGs.find((o) => o.ch_code === 'MACRO_LEVEL')
      ?.outcome_score;
    let scGHG_nat = data.scale_GHGs.find((o) => o.ch_code === 'MEDIUM_LEVEL')
      ?.outcome_score;
    let scGHG_sub = data.scale_GHGs.find((o) => o.ch_code === 'MICRO_LEVEL')
      ?.outcome_score;
    ...
    let scale_GHGs = {
      ...
      data: {
        international: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scGHG_int),
          scGHG_int,
        ),
        national: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scGHG_nat),
          scGHG_nat,
        ),
        subnational: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scGHG_sub),
          scGHG_sub,
        ),
        category_score: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(
            result.outcome_score?.scale_ghg_score,
          ),
          result.outcome_score?.scale_ghg_score,
        ),
      },
    ...
    for (let sd of Object.keys(sdg)) {
      let international = data.scale_SDs.find(
        (o) => o.ch_code === 'MACRO_LEVEL' && o.SDG === sd,
      )?.outcome_score;
      let national = data.scale_SDs.find(
        (o) => o.ch_code === 'MEDIUM_LEVEL' && o.SDG === sd,
      )?.outcome_score;
      ...
      scale_SDs[sd] = {
        ...
        data: {
          international: this.mapNameAndValue(
            this.investorToolService.mapScaleScores(international),
            international,
          ),
          national: this.mapNameAndValue(
            this.investorToolService.mapScaleScores(national),
            national,
          ),
          subnational: this.mapNameAndValue(
            this.investorToolService.mapScaleScores(subnational),
            subnational,
          ),
          category_score: this.mapNameAndValue(
            this.investorToolService.mapScaleScores(cat_score),
            cat_score,
          ),
        },
      };
    ...
    let scAD_int = data.scale_adaptation.find(
      (o) => o.ch_code === 'INTERNATIONAL',
    )?.outcome_score;
    let scAD_nat = data.scale_adaptation.find((o) => o.ch_code === 'NATIONAL')
      ?.outcome_score;
    let scAD_sub = data.scale_adaptation.find(
      (o) => o.ch_code === 'SUBNATIONAL',
    )?.outcome_score;
    ...
    let scale_adaptation = {
      ...
      data: {
        international: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scAD_int),
          scAD_int,
        ),
        national: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scAD_nat),
          scAD_nat,
        ),
        subnational: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scAD_sub),
          scAD_sub,
        ),
        category_score: this.mapNameAndValue(
          this.investorToolService.mapScaleScores(scAD_cat_score),
          scAD_cat_score,
        ),
      },
    };

    let ssGHG_int = data.sustained_GHGs.find((o) => o.ch_code === 'LONG_TERM')
      ?.outcome_score;
    let ssGHG_nat = data.sustained_GHGs.find((o) => o.ch_code === 'MEDIUM_TERM')
      ?.outcome_score;
    let ssGHG_sub = data.sustained_GHGs.find((o) => o.ch_code === 'SHORT_TERM')
      ?.outcome_score;
    ...
    let sustained_GHGs = {
      col_set_1: { label: 'GHG', colspan: 4 },
      data: {
        long_term: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(ssGHG_int),
          ssGHG_int,
        ),
        medium_term: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(ssGHG_nat),
          ssGHG_nat,
        ),
        short_term: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(ssGHG_sub),
          ssGHG_sub,
        ),
        category_score: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(ssGHG_cat_score),
          ssGHG_cat_score,
        ),
      },
    };
    ...
    let sustained_SDs = {};
    for (let sd of Object.keys(sdg)) {
      let long_term = data.sustained_SDs.find(
        (o) => o.ch_code === 'LONG_TERM' && o.SDG === sd,
      )?.outcome_score;
      let medium_term = data.sustained_SDs.find(
        (o) => o.ch_code === 'MEDIUM_TERM' && o.SDG === sd,
      )?.outcome_score;
      ...
      sustained_SDs[sd] = {
        ...
        data: {
          long_term: this.mapNameAndValue(
            this.investorToolService.mapSustainedScores(long_term),
            long_term,
          ),
          medium_term: this.mapNameAndValue(
            this.investorToolService.mapSustainedScores(medium_term),
            medium_term,
          ),
          short_term: this.mapNameAndValue(
            this.investorToolService.mapSustainedScores(short_term),
            short_term,
          ),
          category_score: this.mapNameAndValue(
            this.investorToolService.mapSustainedScores(cat_score),
            cat_score,
          ),
        },
      };
    ...
    let susAD_int = data.sustained_adaptation.find(
      (o) => o.ch_code === 'INTERNATIONAL',
    )?.outcome_score;
    let susAD_nat = data.sustained_adaptation.find(
      (o) => o.ch_code === 'NATIONAL',
    )?.outcome_score;
    let susAD_sub = data.sustained_adaptation.find(
      (o) => o.ch_code === 'SUBNATIONAL',
    )?.outcome_score;
    ...
    let sustained_adaptation = {
      ...
      data: {
        long_term: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(susAD_int),
          susAD_int,
        ),
        medium_term: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(susAD_nat),
          susAD_nat,
        ),
        short_term: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(susAD_sub),
          susAD_sub,
        ),
        category_score: this.mapNameAndValue(
          this.investorToolService.mapSustainedScores(susAD_cat_score),
          susAD_cat_score,
        ),
      },
    };
  ...
  async getAlignmentDataCarbonMarket(
    assessment: Assessment,
    cmScores: any,
    sdgPriorities: SdgPriority[],
  ) {
    ...
    sdgs.map((sd) => {
      ...
      let ans = this.mapNameAndValue(
        this.investorToolService.mapScaleScores(sdg_val),
        sdg_val,
      );
      let priority_value = ans?.value;
      if (sdgPriorities.length !== 0) {
  ...
  async getDashboardData(
    ...
    data
      .select([
        ...
        'asses.to',
        'result.id',
        'result.averageProcess',
        'result.averageOutcome',
      ])
      .leftJoinAndMapOne(
        'asses.climateAction',
        ClimateAction,
    ...
    if (selectedAssessIds && selectedAssessIds.length > 0) {
      data.andWhere('asses.id IN (:selectedAssessIds)', {
        selectedAssessIds: selectedAssessIds,
      });
    }
    data
      .andWhere(filter, { tool, userId, userCountryId, portfolioID })
      ...
      return {
        name:
          (value === 99 || value === -99 || value === '-99'
            ? ''
            : value + ' - ') + name,
        value: value === 99 || value === -99 || value === '-99' ? null : value,
      };
    }
  }
