...
export class CMAssessmentQuestionService extends TypeOrmCrudService<CMAssessmentQuestion> {
  constructor(
    ...
    @InjectRepository(CMDefaultValue)
    private cmDefaultValueRepo: Repository<CMDefaultValue>,
  ...
  async deleteRemovedSDGS(assessmentId: number, selectedSDGS: number[]) {
    ...
    let savedSDGS = await this.sdgAssessmentRepo.find({
    ...
    if (savedSDGS.length > 0) {
      for (let ex of savedSDGS) {
        if (!selectedSDGS.find((o) => o === ex.sdg.id)) {
          ...
          let assessmentQuestion = await this.repo.find({
            where: {
              assessment: { id: assessmentId },
              selectedSdg: { id: ex.sdg.id },
            },
          });
          let qids = assessmentQuestion.map((q) => q.id);
          let assessmentAnswers = await this.assessmentAnswerRepo.find({
            where: { assessment_question: { id: In(qids) } },
          });
  ...
  async saveResult(
    ...
    try {
      for await (let res of results) {
        ...
        if (
          res.selectedSdg?.id !== undefined &&
          !savedSdgs.includes(res.selectedSdg?.id)
        ) {
          let exist = await this.sdgAssessmentRepo.find({
            where: {
              sdg: { id: res.selectedSdg.id },
      ...
      for await (let res of results) {
        ...
        if (res.characteristic?.id === undefined)
          ass_question.characteristic = undefined;
        ...
        if (res.assessmentQuestionId)
          ass_question.id = res.assessmentQuestionId;
        ...
        if (res.type === 'INDIRECT') {
          ...
          if (res.answer && res.answer.id !== undefined) {
            ...
            ass_answer.score =
              (res.answer.score_portion * res.answer.weight) / 100;
          }
          if (res.selectedScore && res.selectedScore.value) {
            ...
            if (res.isSDG) {
              ass_answer.score =
                res.selectedScore.value /
                ((selectedSdgs.length + exists.length) * 3);
            }
          }
          ass_answer.assessment_question = q_res;
      ...
      if (assessment.assessment_approach === 'DIRECT' && !isDraft) {
        ...
        let exist = await this.resultsRepo.findOne({
      ...
      if (expectedGHGMitigation !== undefined) {
        let cm_detail =
          await this.assessmentCMDetailService.getAssessmentCMDetailByAssessmentId(
            assessment.id,
          );
        if (cm_detail) {
          await this.assessmentCMDetailService.updateExpectedGhgMitigation(
            cm_detail.id,
            expectedGHGMitigation,
          );
        }
      }
      return a_ans;
  ...
  async calculateResult(assessmentId: number): Promise<CMScoreDto> {
    ...
    let cmAssessmentQuestions_process = await this.assessmentQuestionRepo
      ...
      .andWhere((qb) => {
        const subQuery = qb
          .subQuery()
          .select('MAX(aq2.id)')
          .from(CMAssessmentQuestion, 'aq2')
          .where('aq2.questionId = aq.questionId')
          .andWhere('aq2.assessmentId = :id')
          .getQuery();
        return `aq.id = ${subQuery}`;
      })
      .getMany();
    ...
    let cmAssessmentQuestions_outcome = await this.assessmentQuestionRepo
      ...
      .where('assessment.id = :id and category.type = "outcome"', {
        id: assessmentId,
      })
      .getMany();

    if (cmAssessmentQuestions_process.length > 0) {
      response.process_score = await this.calculateProcessResult(
        cmAssessmentQuestions_process,
      );
    } else {
    ...
    if (cmAssessmentQuestions_outcome.length > 0) {
      response.outcome_score = await this.calculateOutcomeResult(
        cmAssessmentQuestions_outcome,
        assessmentId,
      );
    } else {
  ...
  async calculateOutcomeResult(
    ...
    const uniqueSdgNamesSet = [
      ...new Set(sdgs.map((assessment) => assessment.sdg.name)),
    ];
    let obj = {};
    let sdgs_score = {};
        ...
        if (cat === 'SCALE_SD' || cat === 'SUSTAINED_SD') {
          ...
          qs.forEach((o) => {
            if (+o.assessmentAnswers[0]?.selectedScore !== -99) {
              if (valid_sdg_score_count[o.selectedSdg?.id])
                valid_sdg_score_count[o.selectedSdg?.id] += 1;
              else valid_sdg_score_count[o.selectedSdg?.id] = 1;
            }
          });
          let valid_scores_sdg = qs.reduce(
            (total, x) =>
              +x.assessmentAnswers[0]?.selectedScore !== -99
                ? total + 1
                : total,
            0,
          );
          ...
          score = qs.reduce((accumulator, object) => {
            let score =
              +object.assessmentAnswers[0]?.selectedScore === -99
                ? 0
                : +object.assessmentAnswers[0]?.selectedScore;
            return accumulator + score;
          }, 0);
          let valid_scores = qs.reduce(
            (total, x) =>
              +x.assessmentAnswers[0]?.selectedScore !== -99
                ? total + 1
                : total,
    ...
    let ghg_score = null;
    let sdg_score = null;
    let adaptation_score = null;
    let scale_ghg_score = null;
    let scale_sdg_score = null;
    let scale_adaptation_score = null;
    let sustained_ghg_score = null;
    let sustained_sdg_score = null;
    let sustained_adaptation_score = null;
    ...
    if (obj['SCALE_ADAPTATION']) {
      ...
      if (adaptation_score !== null) {
        outcomeComponents.push({
          ...
          weight: obj['SCALE_ADAPTATION']?.weight,
        });
      }
    }

    if (outcomeComponents.length > 0) {
      let totalRelevantWeight = outcomeComponents.reduce(
        (sum, c) => sum + c.weight,
        0,
      );
      outcome_score = 0;
  ...
  async getResults(assessmentId: number) {
    let result = [];
    let processData: { technology: any[]; incentives: any[]; norms: any[] } = {
      technology: [],
      incentives: [],
      norms: [],
    };
    ...
    if (questions.length > 0) {
      ...
      let answers2 = await this.assessmentAnswerRepo
        ...
        .innerJoinAndSelect('ans.answer', 'answer', 'answer.id = ans.answerId')
        .innerJoinAndSelect(
          'question.question',
          'cmquestion',
          'cmquestion.id = question.questionId',
      ...
      for (let answ of answers) {
        ...
        if (
          !added_chs.includes(compositeKey) &&
          answ.assessment_question.comment !== null &&
          answ.assessment_question.comment !== undefined
        ) {
      ...
      for await (let ans of uniqueAnswers) {
        ...
        obj.SDG =
          'SDG ' +
          ans?.assessment_question?.selectedSdg?.number +
          ' - ' +
          ans?.assessment_question?.selectedSdg?.name;
        obj.outcome_score = ans?.selectedScore;
        obj.weight =
          ans.assessment_question?.characteristic?.category.cm_weight;
        ...
        if (ans?.assessment_question?.selectedSdg)
          sdgs.push(ans?.assessment_question?.selectedSdg);

        if (
          obj?.category?.code == 'SCALE_GHG' &&
          obj.justification !== undefined &&
          obj.justification !== null
        ) {
          outcomeData.scale_GHGs.push(obj);
        } else if (
          obj?.category?.code == 'SUSTAINED_GHG' &&
          obj.justification !== undefined &&
          obj.justification !== null
        ) {
          outcomeData.sustained_GHGs.push(obj);
        } else if (
          obj?.category?.code == 'SCALE_SD' &&
          obj.justification !== undefined &&
          obj.justification !== null
        ) {
          outcomeData.scale_SDs.push(obj);
        } else if (
          obj?.category?.code == 'SUSTAINED_SD' &&
          obj.justification !== undefined &&
          obj.justification !== null
        ) {
          outcomeData.sustained_SDs.push(obj);
        } else if (
          obj?.category?.code === 'SUSTAINED_ADAPTATION' &&
          obj.justification !== undefined &&
          obj.justification !== null
        ) {
          outcomeData.sustained_adaptation.push(obj);
        } else if (
          obj?.category?.code === 'SCALE_ADAPTATION' &&
          obj.justification !== undefined &&
          obj.justification !== null
        ) {
      ...
      answers2 = [
        ...new Map(
          answers2.map((item) => [
            item.assessment_question.question.code,
            item,
          ]),
        ).values(),
      ];
      ...
      answers2.forEach((ans) => {
        let obj = {
          ...
          relatedQuestions: ans?.assessment_question?.question
            ?.related_questions
            ? JSON.parse(ans?.assessment_question?.question?.related_questions)
            : [],
      ...
      outcomeData.scale_GHGs.sort((a, b) =>
        a.characteristic.localeCompare(b.characteristic),
      );
      outcomeData.scale_SDs.sort((a, b) =>
        a.characteristic.localeCompare(b.characteristic),
      );
      outcomeData.scale_adaptation.sort((a, b) =>
        a.characteristic.localeCompare(b.characteristic),
      );
      outcomeData.sustained_GHGs.sort((a, b) =>
        a.characteristic.localeCompare(b.characteristic),
      );
      outcomeData.sustained_SDs.sort((a, b) =>
        a.characteristic.localeCompare(b.characteristic),
      );
      outcomeData.sustained_adaptation.sort((a, b) =>
        a.characteristic.localeCompare(b.characteristic),
      );
  ...
  checkPreConditions(assessmentAnswers: CMAssessmentAnswer[]) {
    ...
    assessmentAnswers.forEach((ans) => {
      if (
        criterias[ans.assessment_question.question.criteria.id] !== undefined
      ) {
        if (criterias[ans.assessment_question.question.criteria.id]) {
          criterias[ans.assessment_question.question.criteria.id] =
            ans.answer.isPassing;
        }
      } else {
        criterias[ans.assessment_question.question.criteria.id] =
          ans.answer.isPassing;
    ...
    assessmentquestions.map((q) => {
      if (
        [
          ...
          'BEHAVIOUR',
          'AWARENESS',
          'INSTITUTIONAL_AND_REGULATORY',
          'DISINCENTIVES',
          'ECONOMIC_NON_ECONOMIC',
          'BENIFICIARIES',
          'COALITION_OF_ADVOCATES',
          'ENTREPRENEURS',
          'SCALE_UP',
          'ADOPTION',
          'R_&_D',
        ].includes(q.characteristic.code)
      ) {
        if (q.assessmentAnswers[0]?.answer?.id !== undefined) {
          AssessQ.push(q);
        } else if (
          q.assessmentAnswers[0]?.answer?.id === undefined &&
          q.relevance === 0
        ) {
          AssessQ.push(q);
        }
  ...
  async getProcessData(assessmentId: number) {
    ...
    let categories = cmAssessmentQuestions.map(
      (q) => q.characteristic.category,
    );
    ...
    for (let cat of uniqueCats) {
      let qs = cmAssessmentQuestions.filter(
  ...
  async saveTcValue(assessmentId: number, result: CMScoreDto) {
    ...
    if (questions.length > 0) {
      ...
      let answers = await this.assessmentAnswerRepo
        ...
        .getMany();
      let score =
        (result.process_score + result.outcome_score?.outcome_score) / 2;
      let update_score = {};
  ...
  async getDashboardData(
    ...
    try {
      if (intervention_ids && !Array.isArray(intervention_ids)) {
      ...
      let queryResults = await this.resultsRepo
      ...
      if (isUserExternal) {
        queryResults = queryResults.innerJoinAndMapOne(
          'assessment.user',
          Country,
          'user',
      ...
      let formattedResults = await Promise.all(
        filteredResults.map(async (result) => {
          return {
            ...
            assessment_period:
              (result.assessment.from !== null
                ? moment(result.assessment.from).format('DD/MM/yyyy')
                : null) +
              ' - ' +
              (result.assessment.to !== null
                ? moment(result.assessment.to).format('DD/MM/yyyy')
                : null),
          };
        }),
      );
      ...
      if (intervention_ids?.length > 0) {
        ...
        formattedResults = formattedResults.filter((r) =>
          ids.includes(r.assessment),
        );
      }

      interventions_to_filter = this.getDistinctObjects(
        interventions_to_filter,
        'intervention_id',
      );
  ...
  getDistinctObjects(array, property) {
    ...
    return array.filter((obj) => {
      ...
      return uniqueMap.has(propertyValue)
        ? false
  ...
  async getSDGFrequency() {
    ...
    let data = this.repo
      .createQueryBuilder('assessmentQuestion')
      .innerJoin(
        'assessment',
        'assessment',
  ...
  async getSelectedSDGs(assessmnetId: number) {
    return await this.sdgAssessmentRepo
      .createQueryBuilder('sdg')
      .innerJoin(
        'sdg.assessment',
        'assessment',
  ...
  async getDocumentListForReport(
    assessmentId: number,
  ): Promise<CMAssessmentQuestion[]> {
    let result = await this.assessmentQuestionRepo
      ...
      .where('assessment.id = :id and aq.uploadedDocumentPath is not null ', {
        id: assessmentId,
      })
      .getMany();
    return result;
  ...
  async getCMDefaultValues(characteristic_id: number) {
    try {
      return await this.cmDefaultValueRepo
        .createQueryBuilder('default_value')
        .innerJoin(
  ...
  async deleteCMAssessment(assessementId: number) {
    try {
      await this.assessmentCMDetailService.deleteCmAssessmentDetail(
        assessementId,
      );
      let assessmentQuestions = await this.repo
        .createQueryBuilder('cmAssessmentQuestion')
        .innerJoin(
      ...
      if (assessmentQuestions && assessmentQuestions.length > 0) {
        ...
        await this.cMAssessmentAnswerService.deleteAssessmentAnswers(
